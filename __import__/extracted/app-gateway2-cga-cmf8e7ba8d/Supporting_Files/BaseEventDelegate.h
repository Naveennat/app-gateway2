...
class BaseEventDelegate {
    public:
        ...
        bool IsNotificationRegistered(const string& event) {
            string event_l = StringUtils::toLower(event);
            std::lock_guard<std::mutex> lock(mRegisterMutex);
            LOGDBG("Finding notification = %s index=%d", event_l.c_str(), mRegisteredNotifications.find(event_l));
            return mRegisteredNotifications.find(event_l) != mRegisteredNotifications.end();
        }
