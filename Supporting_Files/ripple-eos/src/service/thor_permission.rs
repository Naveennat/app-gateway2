...
use crate::model::permissions::api_grants::{
    ApiGrantRequest, ApiGrantResponse, ApiName, ApiRequestedGrantType, AuthorizationDisposition,
    AuthorizationOutCome,
};
use crate::model::permissions::firebolt::{FireboltPermission, FireboltPermissionRegistry};
use crate::model::permissions::permissions::PermissionResponse;
use crate::model::permissions::permissions::PermissionService;
...
use thunder_ripple_sdk::ripple_sdk::api::distributor::distributor_permissions::PermissionRequestPayload;
use thunder_ripple_sdk::ripple_sdk::api::firebolt::fb_capabilities::CapabilityRole as Role;
...
#[cached(
    key = "String",
    time = 3600,
    convert = r#"{ format!("{}{}", account_session.id, app_id) }"#,
    /*
...
pub async fn cached_get_app_permissions(
    ...
    */
    ...
    let request = tonic::Request::new(EnumeratePermissionsRequest {
        app_key: Some(AppKey {
            app: app_id.clone(),
            syndication_partner_id: account_session.id.clone(),
        }),
        permission_filters,
    });
...
impl ThorPermissionService {
    ...
    pub async fn get_app_permissions(
        ...
        #[cfg(not(test))]
        {
            let channel = match self.get_channel() {
                Ok(c) => c,
                Err(e) => {
                    return Err(ThorPermissionError::ServiceError {
                        context: format!("{:?}", e),
                    })
                }
            };

            cached_get_app_permissions(channel, account_session, app_id, permission_filters).await
        }
    }
    ...
    pub fn firebolt_cap_to_tp_api(&self, capability: &String, role: &Role) -> Vec<String> {
        let found: Vec<FireboltPermission> = self
            ...
            .permissions()
            .into_iter()
            .filter(|perm: &FireboltPermission| {
            ...
            match result {
                Ok(permissions) => {
                    ...
                    match request.payload {
                        Some(PermissionRequestPayload::Check(p)) => {
                        ...
                        Some(PermissionRequestPayload::CheckAll(v_p)) => {
...
pub mod tests {
    use super::*;
    use crate::model::permissions::firebolt::{
    ...
    impl Mockable for ThorPermissionService {
        fn mock() -> Self {
            ThorPermissionService {
                permission_registry: Box::new(SupermanPermissionRegistry),
                grpc_client_session: Arc::<Mutex<GrpcClientSession>>::new(Mutex::new(
                    GrpcClientSession::mock("some_url"),
                )),
            }
        }
    }
    ...
    mod superman_permission_tests {
        use super::*;
        use crate::model::permissions::api_grants::{
            ApiGrantRequest, ApiName, ApiRequestedGrantType,
        };
        use crate::model::permissions::permissions::PermissionResponse;
        use crate::model::permissions::permissions::PermissionService;
        ...
        use thunder_ripple_sdk::ripple_sdk::api::{
            distributor::distributor_permissions::{
                PermissionRequest, PermissionRequestParam, PermissionRequestPayload,
            },
        ...
        async fn setup_superman() -> Box<ThorPermissionService> {
            ...
            //This test is broken because it interacts with live service. It needs mocks
    ...
    pub async fn test_get_app_permission() {
        ...
        let result = test_session.lock().unwrap().get_grpc_channel().clone();
