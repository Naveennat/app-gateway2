...
namespace L0Test {
    ...
    public:
        ...
        string ConfigLine() const override
        {
            ...
            //
            // NOTE: Keep this method free of TRACE()/logging macros. In this repo's L0 build setup,
            // TRACE() expands to code that expects a compatible tracing control context and an
            // instance `this` of a fully instrumented Thunder class. That breaks compilation here.

            return (_cfg.configLineOverride.empty() == false) ? _cfg.configLineOverride : "{\"root\":\"{}\"}";
        }
