...
namespace L0Test {
    ...
    public:
        ...
        {
            // IMPORTANT (L0 stability):
            // Notifications are treated as NON-OWNING pointers.
            //
            // L0 tests often register stack-allocated collectors with a non-standard Release()
            // implementation (may return ERROR_NONE without deleting). Calling AddRef/Release
            // on those objects can cause undefined behavior and teardown crashes.
            //
            // Therefore, we only clear the list; we never Release() notification pointers here.
        ...
        {
            // Minimal validation: context must at least include an appId and connectionId.
            // This aligns with other L0 expectations (bad request vs success).
            if (context.appId.empty() || context.connectionId == 0) {
        ...
        WPEFramework::Core::hresult Emit(const WPEFramework::Exchange::GatewayContext& context,
                                         ...
                                         const string& /*payload*/) override
        {
            if (context.appId.empty() || context.connectionId == 0 || method.empty()) {
                return WPEFramework::Core::ERROR_BAD_REQUEST;
            }
            return _transportEnabled ? WPEFramework::Core::ERROR_NONE : WPEFramework::Core::ERROR_UNAVAILABLE;
        }
        ...
        {
            if (connectionId == 0 || method.empty()) {
                return WPEFramework::Core::ERROR_BAD_REQUEST;
            }
            return _transportEnabled ? WPEFramework::Core::ERROR_NONE : WPEFramework::Core::ERROR_UNAVAILABLE;
        }
        ...
        {
            // Copy the list under lock to avoid reentrancy issues if callbacks register/unregister.
            std::vector<INotification*> sinks;
            {
                std::lock_guard<std::mutex> lock(_notifMutex);
                sinks.assign(_notifications.begin(), _notifications.end());
            }
    ...
    public:
        ...
        void* QueryInterface(const uint32_t id) override
        {
            ...
            // Compatibility note:
            // Some Thunder versions define an interface ID for IShell::ICOMLink, while others
            // (including the installed SDK in this repo) do not. Referencing
            // IShell::ICOMLink::ID will therefore fail to compile on those SDKs.
            //
            // Consumers should obtain the COM link through IShell::COMLink(), which this
            // mock implements (and returns `this`).
            return nullptr;
        }
        ...
        {
            // L0 HARNESS CRITICAL WIRING:
            // Some Thunder SDK variants query the controller shell (callsign == "") for a COM-link style
            // interface using an ID that is *not* consistently defined across SDKs (e.g. RPC::ID_COMLINK).
            //
            // In this repo's isolated L0 harness we don't need strict ID routing here; we only need to ensure
            // that IShell::Root() can reach COMLink()->Instantiate(...) so it instantiates our in-proc fakes
            // (ResponderFake / ResolverFake) rather than loading real implementations.
            //
            // Therefore, if the controller is queried, always expose our ICOMLink implementation.
            if (name.empty() == true) {
                return static_cast<WPEFramework::PluginHost::IShell::ICOMLink*>(this);
            }

            // Not used by current l0tests; return nullptr for all other cases.
            return nullptr;
        }
