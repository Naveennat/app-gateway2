...
# Use a per-run build directory to avoid accidental concurrent deletion/creation races
# (which can manifest as "can't create .o: No such file or directory" during ninja builds).
#
# COMPATIBILITY: Some older harness tooling expects a stable build directory at:
#   ${ROOT}/build/tests_l0_appgateway
# To keep compatibility while still isolating each run, we build into:
...
# Only remove the per-run directory, never the shared parent, to prevent clobbering
# another in-progress run.
mkdir -p "${BUILD_DIR_PARENT}"
rm -rf "${BUILD_DIR}"
mkdir -p "${BUILD_DIR}"
mkdir -p "${COVERAGE_DIR}"

# Update compatibility symlink used by older scripts/logs.
# The stable path can exist either as:
#  - a symlink (expected), or
#  - a real directory (left behind by older runs/manual steps).
# `rm -f` does not remove directories; if we don't handle that, the harness aborts
# early and never reaches runtime tests/artifact generation.
if [[ -L "${STABLE_BUILD_DIR}" ]]; then
  rm -f "${STABLE_BUILD_DIR}"
elif [[ -d "${STABLE_BUILD_DIR}" ]]; then
