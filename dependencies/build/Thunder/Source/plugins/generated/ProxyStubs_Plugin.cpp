...
namespace ProxyStubs {
    ...
    ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_4c4c3f40Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_4c4c3f40 = nullptr;
            ProxyStub::UnknownProxy* parameter_4c4c3f40Proxy = nullptr;
            if (parameter_4c4c3f40Implementation != 0) {
                parameter_4c4c3f40Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_4c4c3f40Implementation, false, _parameter_4c4c3f40);

                ASSERT((_parameter_4c4c3f40 != nullptr) && (parameter_4c4c3f40Proxy != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_4c4c3f40);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_4c4c3f40Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_4c4c3f40Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_7f42ccf3Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_7f42ccf3 = nullptr;
            ProxyStub::UnknownProxy* parameter_7f42ccf3Proxy = nullptr;
            if (parameter_7f42ccf3Implementation != 0) {
                parameter_7f42ccf3Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_7f42ccf3Implementation, false, _parameter_7f42ccf3);

                ASSERT((_parameter_7f42ccf3 != nullptr) && (parameter_7f42ccf3Proxy != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_7f42ccf3);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_7f42ccf3Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_7f42ccf3Proxy, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Register(ICompositPlugin::INotification* _parameter_4c4c3f40) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_4c4c3f40));
        ...
        uint32_t Unregister(ICompositPlugin::INotification* _parameter_7f42ccf3) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_7f42ccf3));
