...
#define APPGATEWAY_L0_INPROC_NO_SHELL_REFCOUNT 1
    ...
    /* virtual */ const string AppGateway::Initialize(PluginHost::IShell* service)
    {
        ...
        // Robust interface acquisition:
        // In this repository's L0 harness we run in-proc with ServiceMock implementing IShell::ICOMLink.
        // Using service->Root<T>() can, depending on Thunder build, load/instantiate the *real* plugin
