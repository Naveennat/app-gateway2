...
namespace Plugin {
    ...
    private:
        ...
        {
            // L0/isolated runs can dispatch this from a worker thread even while the test is
            // tearing down. In that case the IShell pointer (mService) can already be null
            // or otherwise unusable. Never crash here: if we cannot respond, we drop.
            if (mService == nullptr) {
                LOGERR("AppGateway service not available (shutdown/teardown); dropping response");
                return;
            }

            // Try to acquire responder lazily. QueryInterface can return nullptr in the L0 harness.
            if (mAppGatewayResponder == nullptr) {
                mAppGatewayResponder = mService->QueryInterface<Exchange::IAppGatewayResponder>();
            }

            if (mAppGatewayResponder == nullptr) {
                LOGERR("AppGateway Responder not available; dropping response");
                return;
            }
