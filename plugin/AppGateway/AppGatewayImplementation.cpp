    ...
    namespace Plugin
    {
        ...
        string AppGatewayImplementation::UpdateContext(const Context &context, const string& method, const string& params, const string& origin, const bool& onlyAdditionalContext) {
            ...
            if (mResolverPtr->HasIncludeContext(method, additionalContext)) {
                ...
                // L0/JSON-RPC robustness:
                // - Many call paths treat params as optional.
                // - Thunder's JsonObject::FromString("") / FromString("   ") and sometimes even "{}"
                //   can produce noisy parse logs depending on SDK version.
                // Normalize empty/whitespace-only params to "{}" explicitly.
                std::string normalizedParams = params;
                const auto isSpace = [](const char c) -> bool {
                    return (c == ' ') || (c == '\t') || (c == '\n') || (c == '\r');
                };
                bool allSpace = true;
                for (const char c : normalizedParams) {
                    if (!isSpace(c)) { allSpace = false; break; }
                }
                if (normalizedParams.empty() || allSpace) {
                    normalizedParams = "{}";
                }

                JsonObject paramsObj;
                if (!paramsObj.FromString(normalizedParams)) {
                    // Params are optional in JSON-RPC; do not fail the request for a parse issue here.
                    // Keep paramsObj as empty object and continue.
                    LOGWARN("Failed to parse original params as JSON (method=%s). Using empty object.", method.c_str());
                }
