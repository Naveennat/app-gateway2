            ...
            if (!initDone)
            {
                if (nullptr != service)
                {
                    _powerManagerPlugin = PowerManagerInterfaceBuilder(_T("org.rdk.PowerManager"))
                                      .withIShell(service)
                                      .withRetryIntervalMS(FRONTPANEL_PWRMGR_RETRY_INTERVAL)
                                      .withRetryCount(FRONTPANEL_PWRMGR_RETRY_COUNT)
                                      .createInterface();
                }
                ...
                try
                {
...
#if defined(HAS_API_POWERSTATE)
                    {
                        ...
                        if (_powerManagerPlugin) {
                            ...
                            LOGINFO("GetPowerState, pwrStateCur %d, pwrStatePrev %d", pwrStateCur, pwrStatePrev);
                            if (Core::ERROR_NONE == res)
                            {
                                if (WPEFramework::Exchange::IPowerManager::POWER_STATE_ON == pwrStateCur)
                                    powerStatus = true;
                            }
                        }
                        LOGINFO("GetPowerState, powerStatus %d", powerStatus);
                    }
#endif
