...
#define WEBSOCKET_QUEUE_MAX_CNT 10
...
public:
    ...
    public:
        ...
        _queue(WEBSOCKET_QUEUE_MAX_CNT){
    ...
    private:
        ...
        void ProcessMessage(Core::ProxyType<Core::JSONRPC::Message> &message, uint32_t connectionId) {
            // Check for message->Id.IsSet()
                    ...
                    // Forward to automation server after sending if automation is active
                    auto& manager = _parent.Interface();
                    if (manager._automationId > 0 && connectionId != manager._automationId) {
                        std::string params = "{}";
                        if (message->Parameters.IsSet() && !message->Parameters.Value().empty()) {
                            params = message->Parameters.Value();
                        }
                        ...
                        manager.ForwardToAutomation("automationUpdate", automationMsg.str());
                    }
#endif
        }
        ...
        {
            _qLock.Lock();
            if (_queue.Count() == WEBSOCKET_QUEUE_MAX_CNT ) {
...
public:
    ...
    void UpdateConnection(const uint32_t connectionId, const std::string &appId, const bool connected)
    {
#ifdef ENABLE_APP_GATEWAY_AUTOMATION
        // Send connection update to automation server
        if (_automationId > 0) {
            std::stringstream automationMsg;
            automationMsg << R"({"connectionId":)" << connectionId
                         << R"(,"appId":")" << appId << R"("})";
...
#ifdef ENABLE_APP_GATEWAY_AUTOMATION
        if (_automationId > 0 && mChannel != nullptr) {
            Core::ProxyType<Core::JSONRPC::Message> automationMsg = Core::ProxyType<Core::JSONRPC::Message>::Create();
            automationMsg->JSONRPC = Core::JSONRPC::Message::DefaultVersion;
            automationMsg->Designator = designator;
            automationMsg->Parameters = payload;
            mChannel->Submit(_automationId, Core::ProxyType<Core::JSON::IElement>(automationMsg));
