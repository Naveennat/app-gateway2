 ...
 namespace WPEFramework {
     ...
     namespace Plugin {
         ...
         uint32_t Badger::GetWebBrowserStatus(const std::string& appId, std::string& webBrowserStatusJson) {
             ...
             // Use Get/Set for VariantContainer
             std::string bt = platformConfig.Get("browser_type").String();
             if (bt.empty()) bt = "UNKNOWN";
             browserJson.Set("browser_type", bt);

             std::string ver = platformConfig.Get("browser_version").String();
             if (ver.empty()) ver = "UNKNOWN";
             browserJson.Set("version", ver);

             std::string ua = platformConfig.Get("browser_user_agent").String();
             if (ua.empty()) ua = "UNKNOWN";
             browserJson.Set("user_agent", ua);
         ...
         uint32_t Badger::GetNativeDimensions(const std::string& appId, std::string& nativeDimensionsJson) {
             ...
             // Use Set with Variant conversion
             dims.Set("0", Core::JSON::Variant(width));
             dims.Set("1", Core::JSON::Variant(height));

             json.Set("native_dimensions", dims);
         ...
         uint32_t Badger::GetDeviceType(const std::string& appId, std::string& deviceTypeJson) {
             ...
             resultJson.Set("device_type", type);
         ...
         uint32_t Badger::GetDeviceModel(const std::string& appId, std::string& deviceModelJson) {
             ...
             WPEFramework::Core::JSON::VariantContainer resultJson;
             resultJson.Set("device_model", model);
         ...
         uint32_t Badger::DeviceInfo(const std::string& appId, std::string& deviceInfoJson) {
             ...
             capabilities.Set("web_browser", wbObj);

             std::string wifiJson;
             GetWiFiStatus(appId, wifiJson);
             Core::JSON::VariantContainer wifiObj;
             wifiObj.FromString(wifiJson);
             capabilities.Set("is_wifi_device", wifiObj.Get("is_wifi_device"));
             ...
             capabilities.Set("native_dimensions", ndObj);
             ...
             capabilities.Set("video_dimensions", vdObj);
             ...
             capabilities.Set("audio_modes", audioObj);
             ...
             response.Set("device_id", platform.Get("device_id"));
             response.Set("account_id", platform.Get("account_id"));
             capabilities.Set("supports_true_sd", platform.Get("supports_true_sd"));
             capabilities.Set("receiver_platform", std::string("RDK"));
             capabilities.Set("receiver_version", std::string("1.0.0"));

             response.Set("zip_code", std::string("UNKNOWN"));
             response.Set("receiver_id", std::string("UNKNOWN"));
             response.Set("device_hash", std::string("UNKNOWN"));
             response.Set("household_id", std::string("UNKNOWN"));
             WPEFramework::Core::JSON::VariantContainer privacyVC;
             privacyVC.Set("trackingAllowed", false);
             response.Set("privacy_settings", privacyVC);

             response.Set("partner_id", std::string("UNKNOWN"));
             response.Set("user_experience", std::string("UNKNOWN"));

             response.Set("device_capabilities", capabilities);
         ...
         uint32_t Badger::DeviceCapabilities(const std::string& appId, std::string& deviceCapabilitiesJson) {
             ...
             // ---- Audio Modes ----
             {
                 ...
                 capabilities.Set("audio_modes", obj);
             }
             ...
             // ---- Web Browser ----
             {
                 ...
                 capabilities.Set("web_browser", obj);
             }
             ...
             // ---- WiFi ----
             {
                 ...
                 capabilities.Set("is_wifi_device", obj.Get("is_wifi_device"));
             }
             ...
             // ---- Native Dimensions ----
             {
                 ...
                 capabilities.Set("native_dimensions", obj);
             }
             ...
             // ---- Video Dimensions ----
             {
                 ...
                 capabilities.Set("video_dimensions", obj);
             }
             ...
             capabilities.Set("device_type", DelegateUtils::GetStringSafe(platform, "device_type"));
             capabilities.Set("model", DelegateUtils::GetStringSafe(platform, "model"));
             capabilities.Set("supports_true_sd", platform.Get("supports_true_sd"));
         ...
         uint32_t Badger::GetLocalizationPostalCode(const std::string& appId, std::string& postalCodeJson) {
             ...
             WPEFramework::Core::JSON::VariantContainer resultJson;
             resultJson.Set("postal_code", postal);
         ...
         uint32_t Badger::EntitlementsAccountLink(const std::string& appId, const std::string& payload, std::string& result) {
             ...
             const std::string action = payloadVC.HasLabel("action") ? payloadVC.Get("action").String() : "";
             const std::string type   = payloadVC.HasLabel("type") ? payloadVC.Get("type").String() : "";
             ...
             auto buildEntitlementsArray = [&]() -> Core::JSON::ArrayType<Core::JSON::Variant> {
                 ...
                 if (payloadVC.HasLabel("subscriptionEntitlements") &&
                     payloadVC.Get("subscriptionEntitlements").Content() == Core::JSON::Variant::type::ARRAY) {
                     arr = payloadVC.Get("subscriptionEntitlements").Array();
                 }
                 return arr;
             };
             ...
             auto buildIdsObject = [&]() -> Core::JSON::VariantContainer {
                 ...
                 Core::JSON::Variant entVar;
                 entVar = entArr; // use operator= for array -> variant
                 idsObj.Set("entitlements", entVar);
                 return idsObj;
             };
         ...
         */
         ...
         uint32_t Badger::Settings(const std::string& appId, const std::string& payload, std::string& result) {
             ...
             if (request.HasLabel("params") && request.Get("params").Content() == Core::JSON::Variant::type::OBJECT) {
                 Core::JSON::VariantContainer params = request.Get("params").Object();
                 if (params.HasLabel("keys") && params.Get("keys").Content() == Core::JSON::Variant::type::ARRAY) {
                     keys = params.Get("keys").Array();
                 }
             } else if (request.HasLabel("keys") && request.Get("keys").Content() == Core::JSON::Variant::type::ARRAY) {
                 ...
                 if (key == "CC_STATE" || key == "ShowClosedCapture") {
                     ...
                     if (system && system->GetFriendlyName(fnJson) == Core::ERROR_NONE) {
                         ...
                         if (obj.HasLabel("friendly_name")) {
                             const std::string n = obj.Get("friendly_name").String();
                             if (!n.empty()) {
                                 name = n;
                             }
                         }
                     ...
                     wrap.Set("value", valueWithQuotes);
                     merged.Set("friendly_name", wrap);
                 } else if (key == "legacyMiniGuide") {
                     Core::JSON::VariantContainer empty;
                     merged.Set("legacyMiniGuide", empty);
                 } else if (key == "power_save_status") {
                     Core::JSON::VariantContainer empty;
                     merged.Set("power_save_status", empty);
                 } else {
         ...
         uint32_t Badger::LaunchpadAccountLink(const std::string& appId, const std::string& payload, std::string& result) {
             ...
             if (payloadVC.HasLabel("launchpadTile") && payloadVC.Get("launchpadTile").Content() == Core::JSON::Variant::type::OBJECT) {
                 Core::JSON::VariantContainer lp = payloadVC.Get("launchpadTile").Object();
                 if (lp.HasLabel("contentId")) {
                     contentId = lp.Get("contentId").String();
                 }
             }
         ...
         uint32_t Badger::MetricsHandler(const std::string& appId, const std::string& payload, std::string& result) {
             ...
             if (req.HasLabel("evt") && req.Get("evt").Content() == Core::JSON::Variant::type::OBJECT) {
                 Core::JSON::VariantContainer evt = req.Get("evt").Object();
             ...
             if (req.HasLabel("eventType")) {
                 const std::string et = req.Get("eventType").String();

                 if (et == "userAction") {
                     const std::string action = req.HasLabel("action") ? req.Get("action").String() : "";
                     rc = metrics->UserAction(action, args, appId);
                 } else if (et == "appAction") {
                     const std::string action = req.HasLabel("action") ? req.Get("action").String() : "";
                     rc = metrics->AppAction(action, args, appId);
                 } else if (et == "pageView") {
                     const std::string page = req.HasLabel("page") ? req.Get("page").String() : "";
                     rc = metrics->PageView(page, args, appId);
                 } else if (et == "userError") {
                     const std::string errMsg = req.HasLabel("errMsg") ? req.Get("errMsg").String() : "";
                     const bool errVisible = req.HasLabel("errVisible") ? req.Get("errVisible").Boolean() : false;
                     const std::string errCode = req.HasLabel("errCode") ? req.Get("errCode").String() : "";
                 ...
                 Core::OptionalType<std::string> seg;
                 if (req.HasLabel("segment") && req.Get("segment").Content() == Core::JSON::Variant::type::STRING) {
                     seg = req.Get("segment").String();
                 }
