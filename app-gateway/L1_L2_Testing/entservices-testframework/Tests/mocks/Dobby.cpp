...
std::shared_ptr<AI_IPC::IIpcService> AI_IPC::createIpcService(const std::string& address, const std::string& serviceName, int defaultTimeoutMs)
{
    ...
    bool DobbyProxy::hibernateContainer(int32_t descriptor, const std::string& options) const
    {
        EXPECT_NE(impl, nullptr);
        return impl->hibernateContainer(descriptor, options);
    }
    bool DobbyProxy::wakeupContainer(int32_t descriptor) const
    {
        EXPECT_NE(impl, nullptr);
        return impl->wakeupContainer(descriptor);
    }
    bool DobbyProxy::addContainerMount(int32_t descriptor, 
                           const std::string& source, 
                           const std::string& destination,  
                           const std::vector<std::string>& mountFlags, 
                           const std::string& mountData) const
    {
        EXPECT_NE(impl, nullptr);
        return impl->addContainerMount(descriptor, source, destination, mountFlags, mountData);
    }
    
    bool DobbyProxy::removeContainerMount(int32_t descriptor, const std::string& source) const
    {
	EXPECT_NE(impl, nullptr);
        return impl->removeContainerMount(descriptor, source);
    }
    ...
    {
        EXPECT_NE(impl, nullptr);
        return impl->addAnnotation(cd, key, value);
    }
